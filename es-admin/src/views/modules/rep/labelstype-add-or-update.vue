    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.typeId) {
            this.$http({
              url: this.$http.adornUrl(`/rep/labelstype/info/${this.dataForm.typeId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.typeName = data.labelsType.typeName
                this.dataForm.typeRemark = data.labelsType.typeRemark
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/rep/labelstype/${!this.dataForm.typeId ? 'save' : 'update'}`),
              method: 'post',
